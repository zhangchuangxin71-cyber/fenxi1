...
| 字段 | 类型 | 必填 | 默认值 | 说明 |
|---|---:|---:|---:|---|
| `session_id` | string | 是 | 无 | 业务会话与临时文档作用域；会在 SSE 回显并透传检索，但不用于服务端恢复历史 |
| `user_id` | string | 是 | 无 | 用户数据范围，由可信上游提供并透传检索服务 |
...
| `temp_doc_ids` | string[] | 否 | `[]` | 候选临时文档 ID；remote 检索按本请求 `session_id` 校验归属 |
...
服务不根据 `session_id` 保存或恢复历史。调用方每轮都必须通过 `history` 传入需要的
对话上下文，并通过 `report_context` 传入当前大纲和报告；重复使用同一个
`session_id` 只用于业务关联，不会形成 LangGraph checkpoint。
