...
MinerU 是一款文档解析工具，可将 `PDF`、图片以及 `DOC`、`DOCX`、`PPT`、`PPTX`、`XLS`、`XLSX`、`CSV`、Markdown、TXT 转化为机器可读格式（如 Markdown、JSON），便于后续检索、抽取与二次处理。
...
- 支持 `PDF`、图片与 `DOC`、`DOCX`、`PPT`、`PPTX`、`XLS`、`XLSX`、`CSV`、Markdown、TXT 输入；其中旧版 `.doc` 会先转换为 `.docx`，`.ppt` 会先转换为 `.pptx`，`.xls`/`.csv` 会先转换为 `.xlsx`，文本类输入会直接解析后输出
