...
```
> [!TIP]
> - `<input_path>`：本地 `PDF` / 图片 / `DOC` / `DOCX` / `PPT` / `PPTX` / `XLS` / `XLSX` / `CSV` / Markdown / `TXT` 文件或目录
> - `<output_path>`：输出目录
...
- 通过fast api方式调用：
  ...
  >- 上传文件当前支持 `PDF`、图片与 `DOC`、`DOCX`、`PPT`、`PPTX`、`XLS`、`XLSX`、`CSV`、Markdown、`TXT`；旧版 `.doc` 会先转换为 `.docx`，`.ppt` 会先转换为 `.pptx`，`.xls` / `.csv` 会先转换为 `.xlsx`，文本类输入会直接解析后输出
  >
...
- 启动gradio webui 可视化前端：
  ...
  >- WebUI 当前支持上传 `PDF`、图片与 `DOC`、`DOCX`、`PPT`、`PPTX`、`XLS`、`XLSX`、`CSV`、Markdown、`TXT` 文件。
