...
```
> [!TIP]
>- `<input_path>`: Local `PDF` / image / `DOC` / `DOCX` / `PPT` / `PPTX` / `XLS` / `XLSX` / `CSV` / Markdown / TXT file or directory
...
- FastAPI calls:
  ...
  >- Uploads currently support `PDF`, image, `DOC`, `DOCX`, `PPT`, `PPTX`, `XLS`, `XLSX`, `CSV`, Markdown, and TXT files; legacy `.doc` files are converted to `.docx`, `.ppt` files are converted to `.pptx`, `.xls` / `.csv` files are converted to `.xlsx`, and text inputs are parsed directly before output
  >
...
- Start Gradio WebUI visual frontend:
  ...
  >- The WebUI currently accepts `PDF`, image, `DOC`, `DOCX`, `PPT`, `PPTX`, `XLS`, `XLSX`, `CSV`, Markdown, and TXT uploads.
