...
```
> [!TIP]
> `mineru` currently supports local `PDF`, image, `DOC`, `DOCX`, `PPT`, `PPTX`, `XLS`, `XLSX`, `CSV`, Markdown, and TXT file or directory inputs; legacy `.doc` files are converted to `.docx`, `.ppt` files are converted to `.pptx`, `.xls` / `.csv` files are converted to `.xlsx`, and text inputs are parsed directly before output.
