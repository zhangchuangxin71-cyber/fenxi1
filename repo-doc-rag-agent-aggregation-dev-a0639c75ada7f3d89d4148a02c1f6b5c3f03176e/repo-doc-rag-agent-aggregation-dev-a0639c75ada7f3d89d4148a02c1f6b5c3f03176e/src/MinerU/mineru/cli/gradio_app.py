...
def resolve_parse_method(file_path, is_ocr, backend):
    ...
    if file_suffix in office_input_suffixes + text_input_suffixes:
        return "auto"
...
def update_file_options_html(file_path, request: gr.Request, i18n=None):
    ...
    file_suffix = Path(file_path).suffix.lower().lstrip('.')
    is_office = file_suffix in office_input_suffixes
    is_text = file_suffix in text_input_suffixes
...
def update_doc_show(file_path):
    ...
    file_suffix = Path(file_path).suffix.lower().lstrip('.')
    is_office_or_text = file_suffix in office_input_suffixes + text_input_suffixes

    if is_office_or_text:
...
def main(ctx,
    ...
    i18n = gr.I18n(
        en={
            "upload_file": "Select or paste a file to upload\nPDF, image, DOC, DOCX, PPT, PPTX, XLS, XLSX, CSV, Markdown, or TXT",
            "header_title": "MinerU 3: Document Extraction Demo",
            "header_subtitle": "Open-source document extraction for PDF, DOC, DOCX, PPT, PPTX, XLS, XLSX, CSV, Markdown, TXT, and images to Markdown and JSON.",
        ...
        zh={
            "upload_file": "请选择或粘贴要上传的文件\nPDF、图片、DOC、DOCX、PPT、PPTX、XLS、XLSX、CSV、Markdown 或 TXT",
            "header_title": "MinerU 3：文档提取演示",
            "header_subtitle": "开源文档提取工具，支持将 PDF、DOC、DOCX、PPT、PPTX、XLS、XLSX、CSV、Markdown、TXT 和图片转换为 Markdown 与 JSON。",
    ...
    suffixes = [
        f".{suffix}"
        for suffix in pdf_suffixes + image_suffixes + office_input_suffixes + text_input_suffixes
    ]
