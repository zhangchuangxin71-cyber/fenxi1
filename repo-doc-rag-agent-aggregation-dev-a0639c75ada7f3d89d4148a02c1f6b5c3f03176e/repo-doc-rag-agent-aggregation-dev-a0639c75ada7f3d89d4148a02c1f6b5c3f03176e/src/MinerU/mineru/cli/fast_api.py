...
SUPPORTED_UPLOAD_SUFFIXES = (
    pdf_suffixes + image_suffixes + office_input_suffixes + text_input_suffixes
)
    ...
    with zipfile.ZipFile(zip_path, "w", compression=zipfile.ZIP_DEFLATED) as zf:
        for pdf_name in pdf_file_names:
            ...
            if return_original_file:
                ...
                source_dir = Path(parse_dir) / "source"
...
def build_html_url_task_stem(url: str) -> str:
    parsed = urlparse(url)
    host = parsed.netloc or "html"
    path = parsed.path.rstrip("/")
    path_stem = Path(path).stem if path else ""
    raw_stem = f"{host}_{path_stem}" if path_stem else host
    sanitized = re.sub(r"[^0-9A-Za-z._-]+", "_", raw_stem).strip("._-")
    return normalize_task_stem(sanitized or "html")


def assign_task_stems_for_inputs(
    uploads: list[StoredUpload],
    html_urls: list[str],
) -> tuple[list[StoredUpload], list[str]]:
    raw_upload_stems = [upload.stem for upload in uploads]
    raw_html_stems = [build_html_url_task_stem(url) for url in html_urls]
    unique_stems, renamed_stems = uniquify_task_stems(raw_upload_stems + raw_html_stems)

    if renamed_stems:
        logger.warning(
            "Normalized duplicate task stems within request: "
            + ", ".join(f"{source} -> {target}" for source, target in renamed_stems)
        )
    ...
    unique_uploads = [
        ...
        for upload, effective_stem in zip(uploads, unique_stems[:upload_count])
    ]
    return unique_uploads, unique_stems[upload_count:]


def load_parse_inputs(
    uploads: list[StoredUpload],
) -> tuple[list[str], list[bytes], list[str], list[bytes | None], list[str | None]]:
    pdf_file_names = []
    pdf_bytes_list = []
    file_suffixes = []
    original_input_bytes_list = []
    original_input_names_list = []

    for upload in uploads:
        try:
            upload_path = Path(upload.path)
            file_suffix = guess_suffix_by_path(upload_path)
            original_input_bytes = (
                upload_path.read_bytes() if file_suffix in image_suffixes else None
            )
            pdf_bytes = read_fn(upload_path, file_suffix=file_suffix)
        except Exception as exc:
        ...
        original_input_bytes_list.append(original_input_bytes)
        original_input_names_list.append(
            upload.original_name if file_suffix in image_suffixes else None
        )
    return (
        pdf_file_names,
        pdf_bytes_list,
...
async def run_parse_job(
    ...
    (
        ...
        original_input_bytes_list,
        original_input_names_list,
    ) = await asyncio.to_thread(load_parse_inputs, uploads)
    actual_lang_list = normalize_lang_list(request_options.lang_list, len(pdf_file_names))
    html_urls = list(getattr(request_options, "html_urls", []))
    html_stems = list(getattr(request_options, "html_stems", []))
    if len(html_stems) != len(html_urls):
        html_stems = [build_html_url_task_stem(url) for url in html_urls]
    response_file_names = list(pdf_file_names) + list(html_stems)

    parse_kwargs = dict(
        output_dir=output_dir,
        pdf_file_names=list(pdf_file_names),
        pdf_bytes_list=list(pdf_bytes_list),
        html_url_inputs=list(zip(html_stems, html_urls)),
        file_suffixes=list(file_suffixes),
        original_input_bytes_list=list(original_input_bytes_list),
        original_input_names_list=list(original_input_names_list),
...
async def create_async_parse_task(
    ...
    try:
        ...
        uploads, html_stems = assign_task_stems_for_inputs(
            uploads,
            request_options.html_urls,
        )
        file_names = [upload.stem for upload in uploads] + list(html_stems)
        task = AsyncParseTask(
            task_id=task_id,
            status=TASK_PENDING,
            backend=request_options.backend,
            ...
            html_urls=list(request_options.html_urls),
            html_stems=list(html_stems),
        )
    ...
    async def _run_task(self, task: AsyncParseTask) -> None:
        ...
        uploads = [
            ...
            for upload_name, file_name, upload_path in zip(
                task.upload_names,
                task.file_names[: len(task.upload_names)],
                task.uploads,
            )
        ]
...
@app.post(
    path="/file_parse",
    status_code=200,
    summary="Synchronously parse files or HTML URLs",
    description=(
...
@app.post(
    ...
    description=(
        "Submit files or HTML URLs for parsing and return immediately with a task id "
        "that can be checked via the task status and result endpoints."
    ),
)
