    ...
    return [
        VisualizationJob(
            ...
            parse_dir=resolve_parse_dir(
                ...
                backend,
                parse_method,
                is_office=document.suffix in office_input_suffixes,
                is_text=document.suffix in text_input_suffixes,
            ),
            draw_span=draw_span,
        )
        for document in planned_task.documents
        if document.suffix not in office_input_suffixes + text_input_suffixes
    ]
...
def collect_input_documents(
    ...
    for order, path in enumerate(documents):
        ...
        if suffix not in pdf_suffixes + image_suffixes + office_input_suffixes + text_input_suffixes:
            continue
...
@click.option(
    ...
    required=True,
    help="local filepath or directory. support pdf, image, doc, docx, ppt, pptx, xls, xlsx, csv, markdown, txt files",
)
@click.option(
    "-o",
    "--output",
