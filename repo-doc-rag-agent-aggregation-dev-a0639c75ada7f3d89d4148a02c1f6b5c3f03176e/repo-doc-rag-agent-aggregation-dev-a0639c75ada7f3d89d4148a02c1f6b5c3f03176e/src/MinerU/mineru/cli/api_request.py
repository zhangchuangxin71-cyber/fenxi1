...
def validate_html_urls(html_urls: list[str] | None) -> list[str]:
    normalized_urls = []
    for raw_url in html_urls or []:
        for candidate in str(raw_url).splitlines():
            url = candidate.strip()
            if not url:
                continue
            parsed = urlparse(url)
            if parsed.scheme not in {"http", "https"} or not parsed.netloc:
                raise HTTPException(
                    status_code=400,
                    detail=f"Invalid html_urls value: {url}",
                )
...
def normalize_uploaded_files(
    ...
    for value in files or []:
        ...
        if isinstance(value, str):
            # Swagger UI may submit a placeholder string for optional file arrays
            # when the user only fills html_urls.
            if value.strip() in {"", "string"}:
                continue
        raise HTTPException(
            status_code=400,
            detail="Invalid files field. Upload a file or remove the placeholder value.",
        )
...
async def parse_request_form(
    request: Request,
    files: Annotated[
        Optional[list[UploadFile | str]],
        File(
            description=(
                "Upload PDF, image, DOC, DOCX, PPT, PPTX, XLS, XLSX, CSV, Markdown, "
                "or TXT files for parsing. Optional when html_urls is provided."
            ),
    ...
    html_urls: Annotated[
        ...
        Form(
            description=(
                "HTML page URLs to parse through Jina Reader API. Repeat this field "
...
""",
    ...
    return_original_file: Annotated[
        ...
        Form(
            description=(
                "Include the processed original input file and raw uploaded image evidence in the ZIP result; "
    ...
    validated_files = normalize_uploaded_files(files)
    validated_html_urls = validate_html_urls(html_urls)
    if not validated_files and not validated_html_urls:
        raise HTTPException(
            status_code=400,
            detail="At least one uploaded file or html_urls value is required.",
        )
