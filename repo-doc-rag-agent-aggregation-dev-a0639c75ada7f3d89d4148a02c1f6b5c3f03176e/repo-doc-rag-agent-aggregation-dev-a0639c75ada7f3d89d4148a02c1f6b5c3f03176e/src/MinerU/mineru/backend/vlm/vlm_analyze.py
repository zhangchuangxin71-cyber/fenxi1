...
from ...utils.http_vlm_compat import apply_http_vlm_compat
    ...
    def get_model(
        ...
        **kwargs,
        ...
        if backend == "http-client":
            apply_http_vlm_compat()
            server_url = server_url or os.getenv("MINERU_VL_SERVER")
            provider = os.getenv("MINERU_VL_PROVIDER", "").strip().lower()
            is_ark_provider = provider in {"ark", "volcengine", "volces"}
            if is_ark_provider and server_url:
                server_url = server_url.rstrip("/") + "/"
            if is_ark_provider:
                kwargs.setdefault("skip_model_name_checking", True)
        key = (backend, model_path, server_url, os.getenv("MINERU_VL_MODEL_NAME"))
        with self._lock:
            if key not in self._models:
                ...
                model_name = kwargs.get("model_name", None) or os.getenv("MINERU_VL_MODEL_NAME")
