...
DEFAULT_LANG = "txt"
PDF_SIG_BYTES = b'%PDF'
OLE_CFB_SIG_BYTES = b"\xd0\xcf\x11\xe0\xa1\xb1\x1a\xe1"
OLE_CFB_EXTENSION_SUFFIXES = {
    ".doc": "doc",
    ".ppt": "ppt",
    ".xls": "xls",
}
TEXT_EXTENSION_SUFFIXES = {
    ".csv": "csv",
    ".md": "md",
    ".markdown": "markdown",
    ".txt": "txt",
...
def guess_suffix_by_bytes(file_bytes, file_path=None) -> str:
    ...
    if file_path:
        ...
        if (
            path_suffix in OLE_CFB_EXTENSION_SUFFIXES
            and file_bytes.startswith(OLE_CFB_SIG_BYTES)
        ):
            return OLE_CFB_EXTENSION_SUFFIXES[path_suffix]
        if path_suffix in TEXT_EXTENSION_SUFFIXES:
            return TEXT_EXTENSION_SUFFIXES[path_suffix]
...
def guess_suffix_by_path(file_path) -> str:
    ...
    path_suffix = file_path.suffix.lower()
    if path_suffix in OLE_CFB_EXTENSION_SUFFIXES:
        try:
            with open(file_path, "rb") as f:
                if f.read(len(OLE_CFB_SIG_BYTES)) == OLE_CFB_SIG_BYTES:
                    return OLE_CFB_EXTENSION_SUFFIXES[path_suffix]
        except Exception as e:
            logger.warning(f"Failed to read file {file_path} for OLE signature check: {e}")
    if path_suffix in TEXT_EXTENSION_SUFFIXES:
        return TEXT_EXTENSION_SUFFIXES[path_suffix]
