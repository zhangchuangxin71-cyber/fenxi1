...
Converts PDF · DOC/DOCX · PPT/PPTX · XLS/XLSX · CSV · Markdown · TXT · Images · Web pages into structured Markdown / JSON · VLM+OCR dual engine · 109 languages <br>
MCP Server · LangChain / Dify / FastGPT native integration · 10+ domestic AI chip support

**🔍 Core Parsing Capabilities**

- Native support for `DOCX`, `PPTX`, `XLSX`, Markdown, and TXT parsing, with legacy `DOC`, `PPT`, `XLS`, and `CSV` inputs normalized before parsing
...
MinerU is a document parsing tool that converts `PDF`, image, `DOC`, `DOCX`, `PPT`, `PPTX`, `XLS`, `XLSX`, `CSV`, Markdown, and TXT inputs into machine-readable formats such as Markdown and JSON for downstream retrieval, extraction, and processing.
...
- Support `PDF`, image, `DOC`, `DOCX`, `PPT`, `PPTX`, `XLS`, `XLSX`, `CSV`, Markdown, and TXT inputs; legacy `.doc` files are converted to `.docx`, `.ppt` files are converted to `.pptx`, `.xls` / `.csv` files are converted to `.xlsx`, and text inputs are parsed directly before output.
...
`mineru` currently supports local `PDF`, image, `DOC`, `DOCX`, `PPT`, `PPTX`, `XLS`, `XLSX`, `CSV`, Markdown, and TXT file or directory inputs, and can be used for document parsing through the CLI, API, WebUI, and `mineru-router`. Legacy `.doc` files are converted to `.docx`, `.ppt` files are converted to `.pptx`, `.xls` / `.csv` files are converted to `.xlsx`, and text inputs are parsed directly before output. For detailed instructions, please refer to the [Usage Guide](https://opendatalab.github.io/MinerU/usage/).
